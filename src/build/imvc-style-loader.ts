...
const controllerScriptRegexp = /(\\|\/)(controller|Controller)\.(tsx|ts|js|jsx)$/
...
type CompilerStyleInfoWeakMap = WeakMap<webpack.Compiler, Map<string, StyleInfo>>

const compilerStyleInfoWeakMap: CompilerStyleInfoWeakMap = new WeakMap()

const setStyleInfo = (compiler: webpack.Compiler, resourcePath: string, styleInfo: StyleInfo) => {
...
const assetStyleInfo = (compiler: webpack.Compiler, resourcePath: string): StyleInfo => {
    if (!compilerStyleInfoWeakMap.has(compiler)) {
...
const getModule = (module: webpack.compilation.Module): webpack.compilation.Module | undefined => {
...
const travelDownModule = (
    ...
    const queue = (module as any).dependencies.map((dependency: any) => dependency.module)
...
const visitedSetWeakMap = new WeakMap<webpack.compilation.Compilation, Set<webpack.compilation.Module>>()
...
export const handleControllerScriptModule = (
    ...
    const visited = visitedSetWeakMap.get(compilation) as Set<webpack.compilation.Module>
    ...
    const controllerStyleModule: webpack.compilation.Module | undefined = (controllerScriptModule as any).dependencies.find((dependency: any) => {
        if (!dependency.module) {
            return false
        }
        ...
        return controllerStyleRegexp.test(dependencyResourcePath) && path.dirname(dependencyResourcePath) === dirname
    ...
    const controllerStyleInfo = assetStyleInfo(compilation.compiler, controllerStyleResourcePath)
...
export default function ImvcStyleLoader(this: webpack.loader.LoaderContext, content: string) {
    ...
    const getFilename: GetStyleFilenameFn = (newContent) => {
        ...
        // shadow emitFile
        self.emitFile = (filename: string, content: Buffer | string, sourceMap: any) => { }
